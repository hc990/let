from core.middleware.session.redis import Session
from db.redis_interface import Redis  
from tornado.options import options

class SessionMiddleware():
    def __init__(self, request):
        if options.redis_host :
            Redis.create(host=options.redis_host, port=options.redis_port, db=options.redis_db, debug=True)
        else:
